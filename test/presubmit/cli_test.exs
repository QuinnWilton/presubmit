defmodule Presubmit.CLITest do
  use ExUnit.Case, async: true

  alias Presubmit.{CLI, Fixtures}
  ...
  test "without a config file, the defaults are announced with their detection", %{repo: repo} do
    ...
    assert out =~ "No .presubmit.exs; using built-in defaults.\n"
  ...
  test "--rev exits 0 when everything passes or is skipped", %{repo: repo} do
    {0, out} =
      run([
        ...
        "--no-color",
        "--config",
        write_config(repo, "[{Presubmit.Rules.Ecto, []}]")
      ])
  ...
  test "auto picks HEAD on a clean tree and the working tree on a dirty one", %{repo: repo} do
    ...
    assert Presubmit.Git.run!(repo, ["diff", "--cached", "--name-only"]) == ""
  end
  ...
  test "--message-file attaches a message to the staged change set, as a commit-msg hook would",
    ...
    on_exit(fn ->
      Presubmit.Git.run!(repo, ["reset", "-q", "--", "lib/demo/hooked.ex"]) &&
        File.rm(Path.join(repo, "lib/demo/hooked.ex"))
    end)

    Presubmit.Git.run!(repo, ["add", "lib/demo/hooked.ex"])

    message =
      Path.join(System.tmp_dir!(), "presubmit_msg_#{System.unique_integer([:positive])}")
    ...
    {0, out} =
      run([
        ...
        "--no-color",
        "--config",
        write_config(repo, "[Presubmit.Rules.Message]")
      ])
  ...
  test "--base checks the amended commit rather than the delta since HEAD" do
    ...
    repo = Presubmit.Fixtures.repo("shape")
    Presubmit.Git.run!(repo, ["checkout", "-q", "scenario/lib_with_tests"])
    ...
    Presubmit.Git.run!(repo, ["add", "lib/shop/cart.ex"])

    config =
      write_config(repo, "[{Presubmit.Rules.ExUnit, only: [:behaviour_changes_tested]}]")
  ...
  test "--format json", %{repo: repo} do
    ...
    assert out =~
             ~s|{"config":{"defaults":true,"detection":[{"reasons":[],"set":"Presubmit.Rules.Elixir","status":"enabled"}|

    assert out =~
             ~s|{"reasons":["phoenix is a dependency"],"set":"Presubmit.Rules.Phoenix","status":"enabled"}|
  ...
  test "--list prints the configured sets and rules", %{repo: repo} do
    {0, out} =
      run([
        ...
        "--list",
        "--config",
        write_config(repo, "[{Presubmit.Rules.Ecto, except: [:migrations_reversible]}]")
      ])
    ...
    assert defaults =~ "No .presubmit.exs; using built-in defaults.\n"
    assert defaults =~ "Presubmit.Rules.Phoenix\n  routed — "
    assert defaults =~ "  specs (warn) — "
    refute defaults =~ "Presubmit.Rules.Changelog"
  end
  ...
  test "warnings do not fail the run unless --warnings-as-errors", %{repo: repo} do
    ...
    specs_only =
      write_config(repo, "[{Presubmit.Rules.Elixir, only: [:specs], warn: [:specs]}]")
    ...
    scoped = write_config(repo, "[{Presubmit.Rules.Elixir, only: [:specs], in: ~r{^priv/}}]")
  ...
  defp write_config(repo, contents) do
    path =
      Path.join(System.tmp_dir!(), "presubmit_cfg_#{System.unique_integer([:positive])}.exs")
