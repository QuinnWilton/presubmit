defmodule Presubmit.ConfigTest do
  use ExUnit.Case, async: true

  alias Presubmit.{Config, Rules}

  @moduletag :tmp_dir

  describe "default/1" do
    alias Presubmit.{Commit, Tree}
  ...
  test "load!/1 evaluates .presubmit.exs, including inline rule sets", %{tmp_dir: dir} do
    File.write!(Path.join(dir, ".presubmit.exs"), """
    ...
    [
      ...
      {Presubmit.Rules.Shape, max_files: 5},
      ConfigTestRules
    ]
    """)

    config = Config.load!(repo: dir)
    assert config.path == Path.join(dir, ".presubmit.exs")
    ids = Enum.map(config.rules, &{&1.set, &1.id})
  ...
  test "load!/1 reports unknown rules with the file name", %{tmp_dir: dir} do
    File.write!(Path.join(dir, "rules.exs"), "[{Presubmit.Rules.Ecto, only: [:nope]}]")
    error = assert_raise Config.Error, fn -> Config.load!(repo: dir, config: "rules.exs") end
    assert Exception.message(error) =~ "rules.exs: Presubmit.Rules.Ecto has no rule :nope"
  end

  test "load!/1 rejects a file that is not a list, and a missing explicit file", %{tmp_dir: dir} do
    File.write!(Path.join(dir, ".presubmit.exs"), "%{rules: []}")
