defmodule Presubmit.UmbrellaTest do
  ...
  alias Presubmit.{Commit, Config, MixFile, Rules, Tree}
  ...
  test "specs, moduledoc, and behaviour rules see lib/ and test/ inside an app" do
    ...
    assert_raise Presubmit.Violation, ~r/Shop\.Cart\.total\/0/, fn -> assert_specs(commit) end
    assert_raise Presubmit.Violation, ~r/Shop\.Cart/, fn -> assert_moduledoc(commit) end

    assert_raise Presubmit.Violation, ~r/Shop\.Cart\.total\/0 \(added\)/, fn ->
    ...
    assert behaviour_changed?(with_test, Presubmit.Paths.lib())
  end
