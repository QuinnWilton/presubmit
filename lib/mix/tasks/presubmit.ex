defmodule Mix.Tasks.Presubmit do
  ...
  @moduledoc """
  Runs the rules in `.presubmit.exs` (or the built-in defaults) against a
  change set and reports each rule as passed, failed, or skipped.

      mix presubmit                     # working tree if dirty, else HEAD
      mix presubmit --head              # the commit at HEAD
      mix presubmit --rev abc123
      mix presubmit --staged            # from a pre-commit hook
      mix presubmit --staged --message-file "$1"   # from a commit-msg hook: changes and message together
      mix presubmit --worktree          # from an editor or agent loop
      mix presubmit --range main..HEAD  # every commit on a branch
      mix presubmit --format json
      mix presubmit --list
  ...
  def run(argv) do
    ...
    {status, output} = Presubmit.CLI.main(argv)
    IO.write(output)
