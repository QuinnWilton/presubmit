defmodule AssertCommit.Query do
  @moduledoc """
  Functions that read facts out of a `AssertCommit.Commit` for use in
  ...
  structural diff (`AssertCommit.Source.Diff`) and adapter models
  (`AssertCommit.Source.models/2`).
  """

  alias AssertCommit.{Commit, FileChange, Hunk, Message, NoMessageError, Pattern, Tree}
  alias AssertCommit.Source.Diff
  alias AssertCommit.Source.Facts.{Function, Module}
  ...
  def elixir_diff(%Commit{} = commit), do: AssertCommit.Source.diff(commit)
  ...
  @doc "The parsed message, raising `AssertCommit.NoMessageError` if there is none."
