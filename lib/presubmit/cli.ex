defmodule Presubmit.CLI do
  @moduledoc """
  The logic behind `mix presubmit`, separated from Mix so it can be
  ...
  def main(argv, env \\ []) do
    ...
    e in [
      Config.Error,
      Presubmit.GitError,
      Presubmit.MergeCommitError,
      Presubmit.ShallowCloneError
    ] ->
    ...
    {config, reports, notes} =
      case source(opts, repo) do
        {:range, range} ->
          ...
          config =
            load_config(opts, Presubmit.load(repo: repo, source: {:rev, range_end(range)}))
  ...
  defp config_notes(%Config{path: nil, detection: detection}) do
    ...
    ["No .presubmit.exs; using built-in defaults.", "  enabled: #{Enum.join(enabled, ", ")}"] ++
  ...
  defp set_name(module),
    do: module |> inspect() |> String.replace_prefix("Presubmit.Rules.", "")
  ...
  defp source(opts, repo) do
    ...
    case explicit do
      [] ->
        Presubmit.resolve_source(repo: repo, source: :auto)
  ...
  defp list(%Config{sets: sets} = config) do
    "Rules see source shapes only: functions, routes, schemas, and children produced by macros\n" <>
      ...
      Enum.map_join(sets, "\n\n", fn spec ->
        ...
        rules = Presubmit.RuleSet.expand(spec)
  ...
  def usage(error \\ nil) do
    if(error, do: "error: #{error}\n\n", else: "") <>
      """
      Usage: mix presubmit [source] [options]
      ...
      Options:
        ...
        --config PATH     rule configuration (default: .presubmit.exs, or built-in defaults)
