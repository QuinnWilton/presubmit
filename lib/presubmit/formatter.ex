defmodule Presubmit.Formatter do
  @moduledoc """
  ...
  def render(reports, :json, opts) do
    config =
      case Keyword.get(opts, :config) do
        %Presubmit.Config{path: path, detection: detection} ->
          %{
            path: path,
  ...
  defp text(%Report{commit: commit} = report, color?) do
    ...
    exemption =
      case Presubmit.Rule.exemptions(commit) do
        :all ->
          ["  No-Presubmit: true — every rule skipped by the commit's own declaration"]

        [] ->
          []

        ids ->
          ["  Presubmit-Skip: #{Enum.join(ids, ", ")} — skipped by the commit's own declaration"]
      end
  ...
  defp result_lines(
         %Result{rule: rule, outcome: {:error, %Presubmit.RuleTimeoutError{} = e, _}},
         color?
       ) do
