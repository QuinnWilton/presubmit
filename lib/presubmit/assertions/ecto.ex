defmodule Presubmit.Assertions.Ecto do
  @moduledoc """
  Assertions over Ecto schemas and migrations, built on
  `Presubmit.Adapters.EctoSchema` and `Presubmit.Adapters.EctoMigration`.
  ...
  alias Presubmit.Adapters.{EctoMigration, EctoSchema}
  alias Presubmit.Assertions.Flunk
  alias Presubmit.{Commit, Source, Tree}
  ...
  def migrations(%Commit{after: tree}),
    do:
      tree
      |> Source.find(EctoMigration, Presubmit.Paths.migrations())
  ...
  defp published_filter(%Commit{repo: repo}, since) do
    case Presubmit.Git.rev_parse(repo, since <> "^{tree}") do
      {:ok, oid} ->
