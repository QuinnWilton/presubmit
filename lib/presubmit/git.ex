defmodule Presubmit.Git do
  @moduledoc """
  Thin adapter over the `git` plumbing commands presubmit relies on.
  ...
  @doc """
  Runs `git` with `args` inside `repo`, raising `Presubmit.GitError` on
  a non-zero exit.
  ...
  def write_worktree_tree(repo) do
    index =
      Path.join(System.tmp_dir!(), "presubmit_index_#{System.unique_integer([:positive])}")
