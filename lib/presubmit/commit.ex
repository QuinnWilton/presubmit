defmodule Presubmit.Commit do
  ...
  alias Presubmit.{FileChange, Git, Message, Tree}
  alias Presubmit.{MergeCommitError, ShallowCloneError}
  ...
  Raises `Presubmit.MergeCommitError` for merge commits and
  `Presubmit.ShallowCloneError` when the parent is not present locally.
  """
  ...
  `Presubmit.NoMessageError`. On an unborn branch the before tree is empty.
  ...
  The repository's index is not touched (see `Presubmit.Git.write_worktree_tree/1`).
  """
  ...
  """
  @spec restrict(t(), Presubmit.Pattern.t()) :: t()
  def restrict(%__MODULE__{changes: changes} = commit, pattern) do
    kept = Enum.filter(changes, &Presubmit.Pattern.matches?(&1.path, pattern))
