defmodule Presubmit do
  @moduledoc """
  Assertions over the shape and contents of git commits, run as a linter.

  `mix presubmit` loads a change set — a commit, the staged index, or the
  working tree — parses the Elixir it changed into a structural description,
  and runs the rules in `.presubmit.exs` against it. See
  `Mix.Tasks.Presubmit` for the command line and `Presubmit.RuleSet`
  for writing rules.

  Programmatic use:

      commit = Presubmit.load(source: :auto)
      report = Presubmit.Runner.run(commit, Presubmit.Config.load!().rules)
  """

  alias Presubmit.{Commit, Git, Message}
